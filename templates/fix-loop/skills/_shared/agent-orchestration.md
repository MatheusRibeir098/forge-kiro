...
```
O agent terminou quando aparece `ask a question or describe a task` no final da saída (nova UX do Kiro). O prompt `!>` não é mais exibido.
...
- Sempre aguardar `ask a question or describe a task` antes de enviar nova mensagem
